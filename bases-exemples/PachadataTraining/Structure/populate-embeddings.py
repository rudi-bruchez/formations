...
def get_embeddings(tokenizer, encoder: ctranslate2.Encoder, texts: list[str]) -> list[list[float]]:
    ...
    # Convert StorageView (CUDA float16) -> CPU float32 -> NumPy
    sv = output.last_hidden_state
    if sv.dtype != ctranslate2.DataType.float32:
        sv = sv.to(ctranslate2.DataType.float32)
    if sv.device != ctranslate2.Device.cpu:
        sv = sv.to_device(ctranslate2.Device.cpu)
    last_hidden_state = np.array(sv)
        ...
        for i in range(0, total, API_BATCH_SIZE):
            ...
            try:
                ...
                if active_device == "cuda" and is_cuda_loader_issue(exc):
                    ...
                    import traceback
                    print("CUDA execution failed during inference; detailed error:")
                    ...
                    import traceback
                    print("Unexpected embedding error:")
                    print("".join(traceback.format_exception(type(exc), exc, exc.__traceback__)))
                    raise
